        ...
        return{
            ...
            play_area_manner: {
                ...
                three_words_now: {
                    ...
                    judgeId: 195,
                    code3:'7xc_3zx_3zx',
                    Ename: "three_words_now",
                },
            },

            // 玩法菜单管理对象    Ename通过从后台获取的数据中的id3与配置对象中的id比对然后获取相应的
            menu: [
                //			{oneType:'前一',twoType:[{name:'直选复式',Ename:'qianyi'}]},
                //			{oneType:'前二',twoType:[{name:'直选复式',Ename:'qianer'}]},
                //			{oneType:'前三',twoType:[{name:'直选复式',Ename:'qiansan'}]},
                //			{oneType:'定位胆',twoType:[{name:'定位胆',Ename:'dingweidan'}]},
            ],
    ...
    downRefresh() {
      ...
      // },
        initDom:function(){
            ...
            // 点击近期开奖弹出近期开奖列表
            // $(".btn_banner").click(function() {$(".record").slideToggle();$(".ico").toggleClass('active');});
            // 点击遮罩隐藏玩法说明弹出层
...
// 获取系统配置投注项
      ...
      this.closeMenu();
        ...
        handleAddClass: function(item, index, type) {
          ...
          var returnMsg = this.gameRule(type);
          if (returnMsg) {
            mui.toast(returnMsg)
            return
          }
        ...
        gameRule:function(type){
          var _this = this,
            id3 = _this.present_playId,
            count = 0,
            nameList = ["kilobit", "hundreds", "decade", "unit", "numberArr"],num = 0,
            numList = [$(".num1 .second li span.sel_num").length, $(".num2 .second li span.sel_num").length, $(".num3 .second li span.sel_num").length, $(".num4 .second li span.sel_num").length, $(".num5 .second li span.sel_num").length,];
          for (var i = 0; i < 4; i++) {
            if (numList[i] > 0) {
              num++;
            }
        ...
        handlePlayType: function(typeName) {
            var _this = this,
                nameList = ["kilobit", "hundreds", "decade", "unit"];
      ...
      multifunctional_btnClick: function (type, index) {
        ...
        //测试玩法规则
        var returnMsg=this.gameRule(type-1);
        if (returnMsg && index!=5){
          mui.toast(returnMsg)
          return
        }
        switch (index) {
    ...
    watch: {
      ...
      singleCoins:function(val){
        ...
        $route(to,from){
            ...
            if ((to.name == "qxc" && from.name == "depositFile") || (from.name == "qxc" && to.name == "depositFile")) {
              $('.mask').css({ 'display': 'none' });
            } else {
              $('#header').show();
              $('#mainArea').css('display', 'block');
