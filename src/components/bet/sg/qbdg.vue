...
<div id="qbhh">
				...
				<header id="header">
					<div class="gameplaySelect" v-cloak>
						...
						<ul class="gameplayArea clearfix" style="display: none">
							<li @click="changeLoad(30,$event)" data-index="30">{{$t('胜负')}}</li>
							<li @click="changeLoad(31,$event)" data-index="31">{{$t('让分胜负')}}</li>
							<li @click="changeLoad(32,$event)" data-index="32">{{$t('大小分')}}</li>
							<li @click="changeLoad(33,$event)" data-index="33">{{$t('胜分差')}}</li>
							<li @click="changeLoad(29,$event)" data-index="29" style="width: 10rem;">{{$t('混合投注')}}</li>
						</ul>
					</div>
							...
							<li>
								<div class="content">
									<div id='mainContent'>
										...
										<!--单关-->
										<dl v-if="playType==29" class="match-list">
											<dt class="match-divider" @click="fold_dtHeader($event)" :data-index="indexs"><span class="match-date" v-cloak>{{items.titleDateTime}}</span><em class="match-num">&nbsp;&nbsp;{{items.dataList.length}}场</em><span>{{$t('比赛可投')}}</span><i class="match-foldBtn arrow-ico"></i></dt>
											<dd :key="index" v-if="league_indexList[item.leagueIndex]==1" class="match-item represent" :data-index="item.match_id" :data-sign="item.signNum" v-for="(item,index) of items.dataList">
												<div class="league-nd-time jchh_league-nd-time"><span class="row league-name" v-cloak>{{item.league_name}}</span><span class="row league-num">{{item.sessions}}</span><span class="row league-time">{{item.hours}}:{{item.minutes}}截止</span><span class="detail" @click="skip_detail(item.match_id)"></span></div>
												<div class="options-block jchh-options">
													<span class="game-title row">
														<span class="col guestteam" :data-index="item.away_team_id">
														    <em v-cloak>{{item.away_team_name}}</em>
													    </span>
														<span class="col vs">VS</span><span class="col hostteam" :data-index="item.home_team_id">
			    											<em v-cloak>{{item.home_team_name}}</em>
														</span>
												   </span>
										...
										<dl v-if="playType==30" class="match-list">
											<dt class="match-divider" @click="fold_dtHeader($event)" :data-index="indexs"><span class="match-date" v-cloak>{{items.titleDateTime}}</span><em class="match-num" v-cloak>&nbsp;&nbsp;{{items.dataList.length}}场</em><span>{{$t('比赛可投')}}</span><i class="match-foldBtn arrow-ico"></i></dt>
											<dd :key="index" v-if="league_indexList[item.leagueIndex]==1" class="match-item  match-item-01  represent" :data-index="item.match_id" :data-sign="item.signNum" v-for="(item,index) of items.dataList">
												<div class="league-nd-time"><span class="row league-name">{{item.league_name}}</span><span class="row league-num" v-cloak>{{item.sessions}}</span><span class="row league-time">{{item.hours}}:{{item.minutes}}截止</span><span class="detail" @click="skip_detail(item.match_id)"></span></div>
										...
										<dl v-if="playType==31" class="match-list">
										...
										<dl v-if="playType==32" class="match-list">
											<dt class="match-divider" @click="fold_dtHeader($event)" :data-index="indexs"><span class="match-date">{{items.titleDateTime}}</span><em class="match-num">&nbsp;&nbsp;{{items.dataList.length}}场</em><span>{{$t('比赛可投')}}</span><i class="match-foldBtn arrow-ico"></i></dt>
											<dd :key="index" v-if="league_indexList[item.leagueIndex]==1" class="match-item represent" :data-index="item.match_id" :data-sign="item.signNum" v-for="(item,index) of items.dataList">
												<div class="league-nd-time"><span class="row league-name">{{item.league_name}}</span><span class="row league-num">{{item.sessions}}</span><span class="row league-time">{{item.hours}}:{{item.minutes}}截止</span><span class="detail" @click="skip_detail(item.match_id)"></span></div>
										...
										<dl v-if="playType==33" class="match-list">
											<dt class="match-divider" @click="fold_dtHeader($event)" :data-index="indexs"><span class="match-date">{{items.titleDateTime}}</span><em class="match-num">&nbsp;&nbsp;{{items.dataList.length}}场</em><span>{{$t('比赛可投')}}</span><i class="match-foldBtn arrow-ico"></i></dt>
											<dd :key="index" v-if="league_indexList[item.leagueIndex]==1" class="match-item match-item-01 represent" :data-index="item.match_id" :data-sign="item.signNum" v-for="(item,index) of items.dataList">
												<div class="league-nd-time"><span class="row league-name">{{item.league_name}}</span><span class="row league-num">{{item.sessions}}</span><span class="row league-time">{{item.hours}}:{{item.minutes}}截止</span><span class="detail" @click="skip_detail(item.match_id)"></span></div>
						...
						<div class="box-method">
							<div class="count">
								<span @click="freeArea_open($event)">
									<i class="gameplay" v-cloak>{{tzType_free_html}}</i>
									<i class="arrow-ico"></i>
								</span>
								<div class="multiple">
			...
			<div id='sharkit' @click="sharkitFn" :class="(playType==29||playType==28||playType==33)?'hide':''"></div>
...
<script src="../../../assets/js/sg/qbdg.js"></script>
