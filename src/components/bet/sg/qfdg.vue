  ...
  <div class="markHL hhtz" id="hhtz">
    <div id="qfhh">
			...
			<article id="mainArea">
				...
				<header id="header">
					<!--<ul class="">-->
						<!--<li  v-cloak :class="playType<7?'active':''" @click="shift_hh_dg($event,1)"><a>竞足-混合</a></li><li v-cloak :class="playType>6?'active':''" @click="shift_hh_dg($event,2)"><a>竞足-单关</a></li>-->
					<!--</ul>-->
					<div class="gameplaySelect" v-cloak>
						...
						<ul class="gameplayArea clearfix" style="display: none">
							<li @click="changeLoad(8,$event)" data-index="8">{{$t('胜平负')}}</li>
							<li @click="changeLoad(9,$event)" data-index="9">{{$t('让球')}}</li>
							<li @click="changeLoad(10,$event)" data-index="10">{{$t('比分')}}</li>
							<li @click="changeLoad(11,$event)" data-index="11">{{$t('总进球')}}</li>
							<li @click="changeLoad(12,$event)" data-index="12">{{$t('半全场')}}</li>
							<li @click="changeLoad(7,$event)" data-index="7">{{$t('混合投注')}}</li>
						</ul>
					</div>
									...
									<div id='mainContent' :class="playType==9?'rqspf':''" v-cloak>
										...
										<dl :key="indexs" :class="indexs==0?'match-list':'match-list hideOneDay'" v-for="(item,indexs) in dealData" v-cloak>
											<template v-if="playType==1">
												...
												<dd :key="index" v-if="league_indexList[obj.leagueIndex]==1" class="match-item represent spf" v-for="(obj,index) in item.dataList" :data-sign="obj.signNum" :data-index="obj.match_id" :data-type="index">
												...
												<div class="league-nd-time">
													...
													<span class="detail" :data-id="obj.event_id" :data-away="obj.away_team_name" :data-home="obj.home_team_name" :data-away-id="obj.away_team_id" :data-home-id="obj.home_team_id"  :data-index='index'></span>
												      ...
												      <div class="options-block spf-options ">
													       	...
													       	<template v-else> -->
													       		<span @click="tzItem_click($event,indexs,index,'spf',0)" :class="obj.home_win ? 'col betbtn' : 'col unable'" data-index="0" data-gametype="spf"  :data-sp="obj.home_win"><span class="row hostteam" :data-index="obj.home_team_id">{{obj.home_team_name}}</span> <span class="row betinfo">主胜{{obj.home_win ? obj.home_win : "0.00"}}</span></span>
													       		<span @click="tzItem_click($event,indexs,index,'spf',1)" :class="obj.home_draw ? 'col betbtn' : 'col unable'" data-index="1" data-gametype="spf" :data-sp="obj.home_draw"><span class="row vs">VS</span><span class="row betinfo">平{{obj.home_draw ? obj.home_draw : "0.00"}}</span></span>
													       		<span @click="tzItem_click($event,indexs,index,'spf',2)" :class="obj.home_lose ? 'col betbtn' : 'col unable'" data-index="2" data-gametype="spf" :data-sp="obj.home_lose"><span class="row guestteam" :data-index="obj.away_team_id">{{obj.away_team_name}}</span><span class="row betinfo">主负{{obj.home_lose ? obj.home_lose : "0.00"}}</span></span>
													       		<!--<span class="row" data-index="0"><span class="col game-type">0</span><span :class="obj.home_win ? 'col betbtn' : 'col unable'" data-index='0' data-gametype="spf" :data-sp="obj.home_win" @click="tzItem_click($event,indexs,index,'spf_rqspf',0)">主胜{{obj.home_win ? obj.home_win : "0.00"}}</span><span :class="obj.home_draw ? 'col betbtn' : 'col unable'" data-index='1' data-gametype="spf" :data-sp="obj.home_draw" @click="tzItem_click($event,indexs,index,'spf_rqspf',1)">平{{obj.home_draw ? obj.home_draw : "0.00"}}</span><span :class="obj.home_lose ? 'col betbtn' : 'col unable'" data-index='2' data-gametype="spf" :data-sp="obj.home_lose" @click="tzItem_click($event,indexs,index,'spf_rqspf',2)">主负{{obj.home_lose ? obj.home_lose : "0.00"}}</span></span>-->
													       	<!-- </template> -->
														</div>
                          </dd>
											</template>
											<template v-if="playType==9">
												<dt :class="indexs==0?'match-divider':'match-divider close'" :data-index="indexs"><!--// :data-index="item.dateTime[0]"-->
												<span class='match-date'>{{item.titleDateTime}}</span><em class='match-num'>{{item.dataList.length &lt; 10 ? '&nbsp;&nbsp;' + item.dataList.length : item.dataList.length + '场'}}</em><span>{{$t('比赛可投')}}</span><i class="match-foldBtn arrow-ico"></i>
												</dt>
												<dd :key="index" v-if="league_indexList[obj.leagueIndex]==1" class="match-item represent spf" v-for="(obj,index) in item.dataList" :data-sign="obj.signNum" :data-index="obj.match_id" :data-type="index">
												...
												<div class="options-block spf-options ">
													<span :class="parseInt(obj.letball_number) > 0 ? 'col rqCol fuRq c_e24949' : 'col rqCol fuRq'" :data-index="obj.letball_number">{{$t('让')}}<br>{{$t('球')}}<br>{{obj.letball_number > 0 ? "+" + obj.letball_number : obj.letball_number}}</span>
													<span @click="tzItem_click($event,indexs,index,'rqspf',0)" :class="obj.letball_win ? 'col betbtn' : 'col unable'" data-index="0" data-gametype="rfspf" :data-sp="obj.letball_win">
														<span class="row hostteam" :data-index="obj.home_team_id">{{obj.home_team_name}}</span>
														<span class="row betinfo">主胜{{obj.letball_win ? obj.letball_win : "0.00"}}</span>
													</span>
													<span @click="tzItem_click($event,indexs,index,'rqspf',1)" :class="obj.letball_draw ? 'col betbtn' : 'col unable'" data-index="1" data-gametype="rfspf" :data-sp="obj.letball_draw">
														<span class="row vs">VS</span>
														<span class="row betinfo">平{{obj.letball_draw ? obj.letball_draw : "0.00"}}</span>
													</span>
													<span @click="tzItem_click($event,indexs,index,'rqspf',2)" :class="obj.letball_lose ? 'col betbtn' : 'col unable'" data-index="2" data-gametype="rfspf" :data-sp="obj.letball_lose">
														<span class="row guestteam" :data-index="obj.away_team_id">{{obj.away_team_name}}</span>
														<span class="row betinfo">主负{{obj.letball_lose ? obj.letball_lose : "0.00"}}</span>
													</span>
												</div>
                        </dd>
												       <!--<div class="match-content mixall-block">
											...
											<template v-if="playType==10">
												<dt :class="indexs==0?'match-divider':'match-divider close'" :data-index="indexs"><!--// :data-index="item.dateTime[0]"-->
												<span class='match-date'>{{item.titleDateTime}}</span><em class='match-num'>{{item.dataList.length &lt; 10 ? '&nbsp;&nbsp;' + item.dataList.length : item.dataList.length + '场'}}</em><span>{{$t('比赛可投')}}</span><i class="match-foldBtn arrow-ico"></i>
												</dt>
												<dd :key="index" v-if="league_indexList[obj.leagueIndex]==1" class="match-item  match-item-01  represent" v-for="(obj,index) in item.dataList" :data-sign="obj.signNum" :data-index="obj.match_id" :data-type="index">
												<div class="league-nd-time">
													<span class="row league-name">{{obj.league_name}}</span>
													<span class="row league-num">{{obj.sessions ? obj.sessions.length > 3 ? String(obj.sessions).substr(-3) : obj.sessions : ""}}</span>
													<span class="row league-time">{{obj.hours + ':' + obj.minutes + ' 截止'}}</span>
											...
											<template v-if="playType==7">
												<dt :class="indexs==0?'match-divider':'match-divider close'" :data-index="indexs"><!--// :data-index="item.dateTime[0]"-->
												<span class='match-date'>{{item.titleDateTime}}</span><em class='match-num'>{{item.dataList.length &lt; 10 ? '&nbsp;&nbsp;' + item.dataList.length : item.dataList.length + '场'}}</em><span>{{$t('比赛可投')}}</span><i class="match-foldBtn arrow-ico"></i>
												</dt>
												<dd :key="index" v-if="league_indexList[obj.leagueIndex]==1" class="match-item represent" v-for="(obj,index) in item.dataList" :data-sign="obj.signNum" :data-index="obj.match_id" :data-type="index">
												<div class="league-nd-time">
													<span class="row league-name">{{obj.league_name}}</span>
													<span class="row league-num">{{obj.sessions ? obj.sessions.length > 3 ? String(obj.sessions).substr(-3) : obj.sessions : ""}}</span>
													<span class="row league-time">{{obj.hours + ':' + obj.minutes + ' 截止'}}</span>
			...
			<div id="showSelectContent" class="hide">
				...
				<div :class="playType==9?'rqspf selectContent':'selectContent'">
					<div>
						<p>{{$t('投注单')}}</p>
					</div>
			...
			<div id='sharkit' @click="sharkitFn" v-show="playType!==7&&playType!==10&&playType!==13"></div>
...
<script src="../../../assets/js/sg/qfdg.js"></script>
