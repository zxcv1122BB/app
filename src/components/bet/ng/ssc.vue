...
<div>
  ...
  <div id="ssc">
		                ...
		                <p class="banner_num">
                            <span v-if="previousIssue_tips" class="red"  style="font-size:1.2rem;line-height:2.5rem;">开奖中...</span>
			                    ...
			                    <span class="r_bage">{{recentlyNum.split(',')[2]}}</span>
			                     <span class="r_bage">{{recentlyNum.split(',')[3]}}</span>
			                    <span class="r_bage">{{recentlyNum.split(',')[4]}}</span> -->
								...
								<div>
									<ul>
										<li :key="index" v-for="(item,index) in myriabit">
											<span :class="{'sel_num':item.isSel}" @touchstart="showBigSelectBall($event,item)" @touchend="hideBigSelectBall($event)" @touchcancel="hideBigSelectBall($event)">{{item.num}}</span>
										</li>
									</ul>
								</div>
							</dd>
        ...
        <div class="pay" style="display: none">
            ...
            <div class="bets_list">
                <div :key="index" class="bets_item line" v-for="(item,index) in BetsList">
                    <div class="bets_type">
