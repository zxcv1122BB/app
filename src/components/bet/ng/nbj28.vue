                    ...
                    <div class="mui-scroll" style="height: calc(100% - 44px) !important;">
                        <div style="padding: 0 15px">
                            <ul class="mui-row" style="padding: 5px 0;">
                                <li style="line-height: 30px;color: #fff;font-size: 1rem;letter-spacing: 0;float:left;">
                                    {{previousIssue}}期
                            ...
                            <div class="num banner_num" @click="showHis(1)">
                                <!-- <div style="height:1.5rem;overflow:hidden;" id="scrArea">
                                    <ul style="width: 3rem;float: left;height: 2rem;position: relative;">
                                        <li v-bind:key="index" v-for="(r_num,index) in r_list"><span :class="['r_bage',colorList[r_num]]">{{r_num}}</span></li>
                                    </ul>
                                ...
                                <!-- <span class="red" style="font-size: 1.2rem;color:red;height:1.5rem;line-height:1.5rem;" v-if="previousIssue_tips">开奖中...</span> -->
                                <span v-else-if="!recentlyNum||recentlyNum.length==0" class="red" style="height:1.5rem;line-height:1.5rem;">数据获取中...</span>
                                <template v-else>
                            ...
                            <div class="mui-row" style="padding: 10px 0 7px;">
                                ...
                                <div class="mui-col-xs-5 mui-col-sm-5" style="padding-left:10px;" id="push ">
                        ...
                        <div class="body" id="mainArea">
                            <div class="mui-row" style="overflow: auto;height: calc(100% - 70px);background: rgb(203, 212, 222);">
                                <div id="segmentedControlContents" class="mui-col-xs-12 mui-col-sm-12">
                                    ...
                                    <div id="content6" class="mui-control-content right second"><h2>{{right_title}}</h2><div class="content"><div class="content-item square"  :key="index" v-for="(item,index) in numberArr">
                                        <!-- <p  v-if="present_playId==157||present_playId==158" :class="{'sel_num':item.isSel}"  @touchstart="showBigSelectBall($event,item)"  @touchend="hideBigSelectBall($event)"  v-cloak> -->
                                            <p  v-if="present_playId==157||present_playId==158" :class="{'sel_num':item.isSel}"  @click="handleAddClass(item)"  v-cloak>
                                            <span v-cloak>{{item.name}}</span>
                                            <em v-if="isRate" v-cloak>赔率 : {{item.bet}}</em>
                                        </p>
                                        ...
                                        <!-- <p><span :class="{'sel_num':item.isSel}" @touchstart="showBigSelectBall($event,item)" @touchend="hideBigSelectBall($event)" @touchcancel="hideBigSelectBall($event)">{{item.num}}</span></p> -->
                                        </div></div></div>
                                </div>
                            </div>
                        </div>
                        <!-- <div class="mui-text-right" style="position: absolute;bottom: 0;left: 0;width: 100%;padding: 5px 10px;padding-bottom: 15px;">
                            <div class="mui-row">
                                <div class="mui-col-xs-12 mui-col-sm-12">
                                        <input  pattern="[0-9]*" type="number" class="mui-input mui-input-clear"  v-model="singleCoins" @keyup="handleCoins"  placeholder="请输入金额" style="background-color: #485381;color: #eee;font-size: 16px;font-weight: 300;border: 1px solid #555;padding: 6px 15px;margin-top: 3px;height: auto !important;margin-bottom:5px;">
                                    </div>
                            </div>
                            <div class="mui-row "> -->
                            <div class="mui-row mui-text-right" style="position: absolute;bottom: 0;left: 0;width: 100%;padding: 5px 10px;">
                                ...
                                <div class="mui-col-xs-3 mui-col-sm-3" style="color:rgb(221, 221, 221);text-align:center;">
                                    <span class="bl" style="position: absolute;top: 50%;margin-top: -0.8rem;width: 100%;left: 0;text-align: center;">{{bets}}注</span>
                                </div>
...
.afterBet{
   ...
   #offCanvasContentScroll .num span{
       margin: 0;
        vertical-align: baseline;
   }
   .record ul li p:first-child::after{
       top: .7rem
   }
</style>
