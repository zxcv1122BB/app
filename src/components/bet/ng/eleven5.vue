                ...
                <div class="recent">
                  <p class="btn_banner" v-if="!previousIssue">
                    ...
                    <p class="banner_num">
                            ...
                            <span v-else class="r_bage" v-for="it in recentlyNum.split(',')">{{it}}</span>
            ...
            <div class="record">
                ...
                <ul>
                    <li v-for="(item,index) in history">
                        <p>{{item.issue}}期</p>
                        <p class="item_num">
                            <span class="r_bage">{{item.luck_number.split(',')[0]}}</span>
                            <span class="r_bage">{{item.luck_number.split(',')[1]}}</span>
                            <span class="r_bage">{{item.luck_number.split(',')[2]}}</span>
                            <span class="r_bage">{{item.luck_number.split(',')[3]}}</span>
                            <span class="r_bage">{{item.luck_number.split(',')[4]}}</span>
                        </p>
                    </li>
                </ul>
            </div>
                ...
                <div class="common">
                    ...
                    <dl class="num5 clearfix bet_area_item bet_area1" v-if="bet_area_manner.myriabit" v-cloak>
                        ...
                        <dd class="second" >
                            <div>
                                <ul>
                                    <li v-for="(item,index) in myriabit">
                                        <span :class="{'sel_num':item.isSel}"  @touchstart="showBigSelectBall($event,item)" @touchend="hideBigSelectBall($event)" @touchcancel="hideBigSelectBall($event)"  :data-index="index">{{item.num}}</span>
                                    </li>
                                </ul>
                            </div>
                        </dd>
                            ...
                            <div>
                                <ul>
                                    <li v-for="(item,index) in kilobit">
                                        <span :class="{'sel_num':item.isSel}" :data-index="index"  @touchstart="showBigSelectBall($event,item)" @touchend="hideBigSelectBall($event)" @touchcancel="hideBigSelectBall($event)"   v-cloak>{{item.num}}</span>
                                    </li>
                                </ul>
                            </div>
                        </dd>
                    ...
                    <dl class="bet_area_item bet_area0 clearfix" v-if="bet_area_manner.common" v-cloak>
                        <dt v-cloak>{{right_title[0]}}</dt>
                        <dd class="first"  v-if="multifunctionalbtn" >
                            <div class="titleBtn" >
                                <ul>
                        ...
                        <dd :class="['second',multifunctionalbtn?'':'moveup']">
                            <div>
                                <ul>
                    ...
                    <dl class="bet_area_item bet_area0 clearfix" v-if="bet_area_manner.common1" v-cloak>
                        <dt v-cloak>{{right_title[1]?right_title[1]:''}}</dt>
                        <dd class="first"   v-if="multifunctionalbtn" >
                            <div class="titleBtn" >
                                <ul>
                                    <li @click="multifunctional_btnClick(0,$event)">{{$t('全')}}</li>
                                    <li @click="multifunctional_btnClick(1,$event)">{{$t('大')}}</li>
                                    <li @click="multifunctional_btnClick(2,$event)">{{$t('小')}}</li>
                                    <li @click="multifunctional_btnClick(3,$event)">{{$t('单')}}</li>
                                    <li @click="multifunctional_btnClick(4,$event)">{{$t('双')}}</li>
                                    <li @click="multifunctional_btnClick(5,$event)">{{$t('清')}}</li>
                                </ul>
                            </div>
                        </dd>
                        <dd :class="['second',multifunctionalbtn?'':'moveup']">
                            ...
                            <div>
                                <ul>
                                    <li v-for="item in numberArr">
                                        <span :class="{'sel_num':item.isSel}" @touchstart="showBigSelectBall($event,item)" @touchend="hideBigSelectBall($event)" @touchcancel="hideBigSelectBall($event)"  v-cloak>{{item.num}}</span>
                                    </li>
                                </ul>
                            </div>
                        </dd>
                ...
                <div class="bets_item line" v-for="(item,index) in BetsList">
                    <div class="bets_type">
                        {{item.type.replace(/[-]+[\u4e00-\u9fa5]*/g,"")}}<span>{{"("+item.type.replace(/[\u4e00-\u9fa5]*[-]+/g,"")+")"}} 共<em class="red">{{item.betsCount}}</em>注，共<em class="red">{{item.betsCoins}}</em>{{coinUnit}}
                        	<!--{{item.odds.length>20?item.odds.substr(0,20)+"...":item.odds}} 倍-->
                        </span>
                    </div>
                    <div class="bets_info">
                        {{item.betsClause}}
...
<!--<script src="../../../api/compute/bettingCountUtils.js"></script>-->
<script src="../../../assets/js/ng/eleven5.js"></script>
